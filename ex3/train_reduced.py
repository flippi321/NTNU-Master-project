...
# Same mapping ex2/shap_analyse.py uses to collapse one-hot dummies to a single
# "logical" feature name.  Keeping it duplicated (not imported) so ex3/ has no
# runtime dependency on ex2/.
ONE_HOT_GROUPS = {
...
def _friendly_name(raw: str) -> str:
    """Same transform as ex2/shap_analyse.py: strip @NT3BL* cohort suffix
...
def select_features(imp_df: pd.DataFrame,
                    ...
                    imp_thr: float,
                    corr_thr: float,
                    top_k: int | None) -> tuple[list[str], dict]:
...
def map_aggregated_to_raw_indices(kept_aggregated: list[str],
                                  raw_feature_names: list[str]) -> tuple[list[int], list[str]]:
    ...
    Indices are returned sorted (preserves raw-column order, so the conditioning
    vector layout is deterministic and human-readable).
    """
    ...
    for agg in kept_aggregated:
        if agg in one_hot_label_to_prefix:
            ...
            # Find the raw column whose friendly name matches the aggregated label.
